...
/*Views code*/
CREATE VIEW vw_gamers AS
SELECT 	playerID AS ID, 
		playerFirstName AS first, 
		playerLastName AS last, 
		playerEmail AS email, 
		teamID AS team,
        playerDOB AS birthday,
        playerAlias AS alias
        
FROM player;

CREATE VIEW vw_playerTeamName AS
SELECT 	playerID AS ID, 
		playerFirstName AS first, 
		playerLastName AS last, 
		playerEmail AS email, 
